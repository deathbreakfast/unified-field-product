...
async fn create_item(
    ...
    IndexedDemoItem::upsert_used(id, row, v, valence::use_!("upsert IndexedDemoItem in workspace_search/demo/tests.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."))
        .await
...
async fn se_create_indexes_document_happy() {
    ...
    let doc = UnifiedFieldSearchDocument::get_used(&id, &sys, valence::use_!("get UnifiedFieldSearchDocument in workspace_search/demo/tests.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."))
        .await
        .expect("get")
        .expect("indexed");
...
async fn se_update_refreshes_document_happy() {
    ...
    .expect("new");
    IndexedDemoItem::upsert_used("d2", updated, &v, valence::use_!("upsert IndexedDemoItem in workspace_search/demo/tests.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."))
        .await
        .expect("update");
    let id = document_id(
        &RecordId::new("user", "alice"),
...
async fn se_delete_removes_document_happy() {
    ...
    let sys = v.with_actor(system());
    assert!(UnifiedFieldSearchDocument::get_used(&id, &sys, valence::use_!("get UnifiedFieldSearchDocument in workspace_search/demo/tests.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."))
        .await
        .expect("get")
        .is_some());
    ...
    assert!(UnifiedFieldSearchDocument::get_used(&id_b, &sys, valence::use_!("get UnifiedFieldSearchDocument in workspace_search/demo/tests.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."))
        .await
        .expect("get")
        .is_none());
}
...
async fn se_error_does_not_fail_source_write_sad() {
    ...
    .expect("new");
    let saved = IndexedDemoItem::upsert_used("derr", row, &v, valence::use_!("upsert IndexedDemoItem in workspace_search/demo/tests.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."))
        .await
    ...
    let sys = v.with_actor(system());
    assert!(
        UnifiedFieldSearchDocument::get_used(&id, &sys, valence::use_!("get UnifiedFieldSearchDocument in workspace_search/demo/tests.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."))
            .await
            .expect("get")
            .is_none(),
...
async fn se_uses_source_user_field_happy() {
    ...
    let doc = UnifiedFieldSearchDocument::get_used(&id, &v, valence::use_!("get UnifiedFieldSearchDocument in workspace_search/demo/tests.rs; Valence persistence for this feature path; typed store; visible to session actor / service path."))
        .await
        .expect("get")
